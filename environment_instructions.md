...
1. Make sure the .yml file is in your current working directory.
2. Create it using the following: 
  - "conda env create -f grl_env.yml"
  - Alternatively, you can just include a relative path to the .yml (e.g. conda env create -f Documents/GRL/grl_env.yml)
...
3. Update the .yml file: 
  - "conda env export -n GRL_env --from-history > grl_env.yml"
...
2. Update the existing environment on your machine:
  - "conda env update -f grl_env.yml --prune"
